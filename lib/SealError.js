...
class SealError extends Error {
  constructor(errorCode, httpStatusCode = 500, metadata = {}) {
    ...
    if (moduleErrors && moduleErrors[errorCode]) {
      ...
      this.code = errorCode;
      this.metadata.kbCode = `${errorCode.replace(/^ERR_/, '')}-${pkg.name.replace(/^(@sealsystems\/|seal-)/, '')}`;
      this.metadata.sealSpaceUrl = `https://sealspace.sealsystems.de/display/KB/${this.metadata.kbCode}`;
